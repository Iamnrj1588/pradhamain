        ...
        {bookings.map((booking) => (
          <div key={booking.id} className="border border-gray-200 rounded-lg p-4">
            ...
            <div className="space-y-2 text-sm">
              <div>
                <span className="font-medium text-gray-700">Dress:</span>
                <span className="ml-2">{booking.dress.name}</span>
              </div>
              <div>
                <span className="font-medium text-gray-700">Size:</span>
                <span className="ml-2">{booking.selectedSize}</span>
              </div>
              <div>
                <span className="font-medium text-gray-700">Dates:</span>
                <span className="ml-2">{booking.startDate} to {booking.endDate}</span>
              </div>
              <div>
                <span className="font-medium text-gray-700">Amount:</span>
                <span className="ml-2 font-semibold">₹{booking.totalAmount}</span>
              </div>
            </div>
            ...
            {booking.status === 'PENDING' && (
              <div className="flex space-x-3 mt-4 pt-3 border-t border-gray-200">
                ...
                <button
                  onClick={() => onUpdateStatus(booking.id, 'CANCELLED')}
                  className="flex-1 bg-red-600 text-white py-2 px-3 rounded-md text-sm font-medium hover:bg-red-700 flex items-center justify-center"
                >
