...
export default function VerifyEmail() {
  ...
  const submitOtp = async () => {
    ...
    try {
      ...
      await axios.post(`${API_URL}/verify-otp`, userData);
      setMessage({ text: "Email verified successfully! Redirecting to login...", type: "success" });
      setTimeout(() => navigate("/login"), 2000);
    } catch (error) {
      setMessage({ text: error.response?.data?.error || "Invalid OTP!", type: "error" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const resendOtp = async () => {
    ...
    try {
      ...
      setMessage({ text: "OTP sent again!", type: "success" });
    } catch {
      ...
      <div className="w-full max-w-md relative z-10">
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-2xl p-8 border border-white/20">
          <div className="text-center mb-8">
            ...
            <h2 className="text-2xl font-bold text-[#8B1538] mb-2">Verify Email</h2>
            <p className="text-gray-600 mb-2">Enter the verification code</p>
            <p className="text-sm text-gray-500">We've sent a code to: <span className="font-medium text-[#8B1538]">{email}</span></p>
          </div>
          ...
          <div className="space-y-6">
            <input 
              ...
              maxLength="6"
              className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#8B1538] focus:border-transparent outline-none transition-all text-center text-lg tracking-widest font-mono"
            />
            ...
            <button 
              onClick={resendOtp}
              disabled={loading}
              className="w-full bg-transparent hover:bg-gray-50 text-[#8B1538] font-semibold py-3 px-4 rounded-lg border border-[#8B1538] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
